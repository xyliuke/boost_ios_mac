...
: ${BOOST_LIBS:="random regex chrono thread signals filesystem system date_time"}
: ${IPHONE_SDKVERSION:=`xcodebuild -showsdks | grep iphoneos | egrep "[[:digit:]]+\.[[:digit:]]+" -o | tail -1`}
: ${OSX_SDKVERSION:=10.9}
: ${XCODE_ROOT:=`xcode-select -print-path`}
: ${EXTRA_CPPFLAGS:="-DBOOST_AC_USE_PTHREADS -DBOOST_SP_USE_PTHREADS -std=c++11 -stdlib=libc++ -DBOOST_NO_CXX11_SCOPED_ENUMS -DBOOST_NO_ALIGNMENT"}
...
BOOST_SRC=$SRCDIR/src/boost_${BOOST_VERSION2}
...
cleanEverythingReadyToStart()
{
	...
	#rm -rf iphone-build iphonesim-build-i386 iphonesim-build-x86_64 osx-build
    rm -rf $IOSBUILDDIR
    rm -rf $OSXBUILDDIR
    rm -rf $PREFIXDIR
...
EOF
    ...
    #./bjam -j16 --build-dir=iphonesim-build-i368 --stagedir=iphonesim-build-i386/stage/i386 --toolset=darwin-${IPHONE_SDKVERSION}~iphonesim architecture=i386 target-os=iphone macosx-version=iphonesim-${IPHONE_SDKVERSION} link=static stage
    #./bjam -j16 --build-dir=iphonesim-build-x86_64 --stagedir=iphonesim-build-x86_64/stage/x86_64 --toolset=darwin-${IPHONE_SDKVERSION}~iphonesim architecture=x86_64 target-os=iphone macosx-version=iphonesim-${IPHONE_SDKVERSION} link=static stage
	doneSection
